...
//=========================================================================
// PUBLIC: WebSocketManager Constructor 
//=========================================================================
...
// 			}
// 		}
// @usage wscObj = new WebSocketManager("localhost", 9010, loginData );
// @return Either a WebSocketManager instance or null
...
(function (root, factory) {
	...
	/**
	 * Constructs the WebSocketManager CLASS - there is ONLY 1 of theses globally!
	 * @constructor
	 */

	WebSocketManagerClass = function() {
		var self = this;
		...
		//=========================================================================
		// PRIVATE: Initialize WebSocketManager
		//=========================================================================
		...
		this._loginSuccessCB = function( evt ) {
			...
			console.localInfoOnly("WSKT", "WebSocketManager: _loginSuccessCB: Auto Login Success");
			// EventBus.dispatch( NcEvents.onRemoteLoggerInfo, ["WSKT", "WebSocketManager: _loginSuccessCB", JSON.stringify( evt, null, '\t' ) ]);

			WebSocketManager.webSocketSessionId = evt.wsSendData.data.session_id;
		...
		this._loginErrorCB = function( evt ) {
			...
			console.localErrorOnly("WSKT", "WebSocketManager: _loginErrorCB: Auto Login ERROR");
			EventBus.dispatch( NcEvents.onRemoteLoggerError, ["WSKT", "WebSocketManager: _loginErrorCB", JSON.stringify( evt, null, '\t' ) ]);

			var msg = evt.wsResponseData.result + "_" + evt.wsResponseData.data.code + "_" + evt.wsResponseData.data.error;
			WebSocketManager.webSocketSessionId = msg;
		...
		// PUBLIC: logPings(...)
		//-------------------------------------------------------------------------
		// @param TrueFalse: Boolean - true or false - 
		//     If True, pings will be put in the logs for showSimpleLogs(...)
		// @see showSimpleLogs(...) 
		//-------------------------------------------------------------------------
		this.logPings = function( TrueFalse ) {
			self.logPings = TrueFalse;
		};
		...
		//   related to WebSocketManager that can or should be called because of events
		//   sent via this mechanism.  
		// On the other hand, Callbacks are used when a response back to WebSocketManager
		...
		this.connect = function( ) {
			...
			if ( this.host === null ) {
				throw( "WebSocketManager: connect: Login Crednetials are not yet set. It MUST be set with WebSocketManager.setConnectData before trying to connect!");
			}
			// TODO ... 
			...
			if ( self.isWebSocketOpen() === false ) {
				...
				var headerData = { 
					codeRev: this.CODE_REV, 
					url: "  wss://:" + self.host + ":" + self.port + "/",
					LoginObj: self.loginObj
				};

				EventBus.dispatch( NcEvents.onRemoteLoggerInfo, ["WSKT", "WebSocketManager: connect: Connecting", JSON.stringify( headerData, null, '\t' ) ]);
		...
		this.close = function() {
			if ( ( !! self.webSocket ) && ( self.webSocket.readyState !== self.WS_STATE_CLOSED ) ) {
				// console.localInfoOnly("WSKT", 'WebSocketManager: close: Closing WS now');
				EventBus.dispatch( NcEvents.onRemoteLoggerInfo, ["WSKT", "WebSocketManager: close: Closing WS now" ]);
				...
				console.localInfoOnly("WSKT", 'WebSocketManager: close: Nothing to close!');
			}
		};
		...
		this._onWsOpen = function(evt) {
			...
			// console.localInfoOnly("WSKT", "WebSocketManager: _onWsOpen ", evt );
			EventBus.dispatch( NcEvents.onRemoteLoggerInfo, ["WSKT", "WebSocketManager: _onWsOpen", JSON.stringify( evt, null, '\t' ) ]);
		...
		this._onWsClose = function(evt) {
			...
			// console.localInfoOnly("WSKT", "WebSocketManager: _onWsClose ", evt );
			EventBus.dispatch( NcEvents.onRemoteLoggerInfo, ["WSKT", "WebSocketManager: _onWsClose", JSON.stringify( evt, null, '\t' ) ]);
			...
			WebSocketManager.webSocketSessionId = "_onWsClose";
		...
		this._onWsError = function(evt) {
			...
			// console.localErrorOnly("WSKT", "WebSocketManager: _onWsError ", evt );
			EventBus.dispatch( NcEvents.onRemoteLoggerError, ["WSKT", "WebSocketManager: _onWsClose", JSON.stringify( evt, null, '\t' ) ]);
			...
			WebSocketManager.webSocketSessionId = "_onWsError";
		...
		this._onWsMessage = function( Event ) {
			...
			// 	console.localLogOnly("WSKT", "WebSocketManager: _onWsMessage ", Event );
			// }

			// console.localDebugOnly("WSKT", "WebSocketManager: _onWsMessage ", Event );
			...
			if ( typeof(JsonData) === 'object' ) {
				...
				if ( JsonData.class === "core" && JsonData.cmd === "ping" ) { 
					...
					// console.localDebugOnly("WSKT", "WebSocketManager: _onWsMessage: Got Ping -> Sending Pong" );
				...
				// =========== RESULT ============
					...
					// console.localLogOnly("WSKT", "WebSocketManager: _onWsMessage: " + ( self.resultPrinter(JsonData) ), JSON.stringify(JsonData, null, "\t") );
					...
					if ( !! self.sentQueue[ JsonData.tid ] ) {
						...
						console.localErrorOnly( "WebSocketManager: _onWsMessage: Got result for something not in queue .. ", JSON.stringify(JsonData, null, "\t") );
					}
				...
				// =========== CLASS ============
					...
					// console.localLogOnly("WSKT", "WebSocketManager: _onWsMessage: CORE : EVENT ", JsonData, recvObj );
					...
					console.localDebugOnly("WSKT", "WebSocketManager: _onWsMessage: *** I got a CLASS of " + ( self.cmdPrinter(JsonData) ), JsonData );

					if ( JsonData.class === "core" ) {
						console.localLogOnly("WSKT", "WebSocketManager: _onWsMessage:  *** I got an CORE CLASS ", JSON.stringify(JsonData, null, "\t") );

						// Handle core classes here in WsConnection ...

					} else {

						console.localLogOnly("WSKT", "WebSocketManager: _onWsMessage: *** I got an OTHER CLASS ", JSON.stringify(JsonData, null, "\t") );
						...
						if ( self.handle( JsonData ) ) {
							...
							console.localErrorOnly("WSKT", "WebSocketManager: _onWsMessage: *** I got an OTHER CLASS ", JSON.stringify(JsonData, null, "\t") );
					...
					console.localErrorOnly("WebSocketManager: Object is not a callback or event ", JSON.stringify(JsonData, null, "\t") );
				}
			} else {
				console.localErrorOnly("WebSocketManager: Error was returned", Event);
			}
		...
		this.sendData = function( StoreData, PutFirst ) {
			...
			if (( typeof(StoreData) === 'object' ) &&  (StoreData !== null ) ) {		// Just push StoreData into 
				...
				if ( !StoreData.ping ) {
					EventBus.dispatch( NcEvents.onRemoteLoggerInfo, ["WSKT", "WebSocketManager: sendData", JSON.stringify(StoreData, null, '\t') ]);
				}
				...
				if ( self.toSend.length > 0 ) {
					...
					if ( !!self.webSocket ) {
						if ( self.webSocket.readyState === self.WS_STATE_OPEN ) {
							...
							// if ( !Data.ping ) {
							// 	EventBus.dispatch( NcEvents.onRemoteLoggerInfo, ["WSKT", "WebSocketManager: sendData", JsonData ]);
							// }
							...
							console.localLogOnly("WSKT",  "WebSocketManager: sendData: WS_STATE_CLOSED -- Trying to Connect " );
							self.connect();
						} else if ( self.webSocket.readyState === self.WS_STATE_CONNECTING ) {
							console.localLogOnly("WSKT",  "WebSocketManager: sendData: WS_STATE_CONNECTING -- We will wait here " );
							// setTimeout( self.sendData, myDate, Data, myDate + 100 );

						} else if ( self.webSocket.readyState === self.WS_STATE_CLOSING ) {
							console.localLogOnly("WSKT",  "WebSocketManager: sendData: WS_STATE_CLOSING -- We will wait here " );
							// setTimeout( self.sendData, myDate, Data, myDate + 100 );
						}
					} else { 	// WebSocket must be null
						console.localLogOnly("WSKT",  "WebSocketManager: sendData: webSocket.readyState FALSE -- Trying to Connect " );
						self.connect();
					}
		...
		this.sendAction = function( Action, PutFirst ) {
			...
			// console.localInfoOnly("WSKT", "WebSocketManager: sendAction: " + Action.wsSendData.cmd, Action.wsSendData.data, JSON.stringify(JsonAction, null, "\t")  );
		...
		this.remoteLog = function( Level, Message, MetaData ) {
			...
			if ( typeof(Level) !== 'number' ) {
				throw( "WebSocketManager: remoteLog: Level MUST be a number!");
			}

			if ( typeof(Message) !== 'string' ) {
				throw( "WebSocketManager: remoteLog: Message MUST be a string!");
			}
			...
			if ( typeof(MetaData) === 'string' ) {
				...
				throw( "WebSocketManager: remoteLog: MetaData MUST be either a JSON formatted string or an object!");
			}

			var logObj = {
				tid: WebSocketManager.evt_cnt++,
				createDate: new Date(),
				...
				_successCB: function( Data, MsgDate ) {	
					...
					// console.localDebugOnly("WSKT", "WebSocketManager: Callback: " + logObj.wsSendData.cmd + "._successCB(" + this.tid + "): " +
					//  	Data.result, Data );
				},
				_errorCB: function( Data, MsgDate ) {
					...
					console.localErrorOnly("WSKT", "WebSocketManager: Callback: " + logObj.wsSendData.cmd + "._errorCB(" + this.tid + "): " +
					 	Data.result, Data );
				}
			...
			if ( self.isWebSocketReady() === false ) {
				...
				self.remoteLogDb.forEach( 
					function( tmpLogObj ) {
						...
						tmpLogObj.wsSendData.data.meta.jsClient_wsStableSessionId = WebSocketManager.getWsSessionId();
						tmpLogObj.wsSendData.data.meta.jsClient_userId = WebSocketManager.userName;
						tmpLogObj.wsSendData.data.meta.jsClient_deviceId = WebSocketManager.deviceId;
						tmpLogObj.wsSendData.data.meta.jsClient_userAgent = WebSocketManager.userAgent;
						tmpLogObj.wsSendData.data.meta.jsClient_wsSendTimeStamp = thisDate.getTime();
						tmpLogObj.wsSendData.data.meta.jsClient_wsSessionIdAtSend = WebSocketManager.jsWebSocketSessionId;
...
// ####################################################################################
	...
	WebSocketManagerClass.prototype = {
		...
		setDebugFlag:function( TrueFalse ) {
		...
		setConnectData:function( HostString, PortNumber, UserNameString, UserPasswordString, DeviceIdString, MetaDataObject ) {
			...
			if ( typeof(HostString) !== 'string' ) { throw("WebSocketManager: setConnectData: HostString must be an String"); }
			if ( typeof(PortNumber) !== 'number' ) { throw("WebSocketManager: setConnectData: PortNumber must be an Number"); }
			if ( typeof(UserNameString) !== 'string' ) { throw("WebSocketManager: setConnectData: UserNameString must be an String"); }
			if ( typeof(UserPasswordString) !== 'string' ) { throw("WebSocketManager: setConnectData: UserPasswordString must be an String"); }
			...
			if ( typeof(MetaDataObject) !== 'undefined' ) {
				if ( typeof(MetaDataObject) === 'object' ) { 
					...
					throw("WebSocketManager: setConnectData: MetaDataObject was provided but it was not an object.  It must be an object if provided!");
				}
			}

			this.jsWebSocketSessionId = WebSocketManager.createUuid();

			this.webSocketSessionId = DeviceIdString;
		...
		updateLoginObj:function( HostString, PortNumber, UserNameString, UserPasswordString, DeviceIdString, MetaDataObject ) {
			...
			WebSocketManager.setConnectData( HostString, PortNumber, UserNameString, UserPasswordString, DeviceIdString, MetaDataObject );
		...
		sendDataViaPromise:function( Class, SubClassName, Command, Data ) {
			return new Promise( function(resolve, reject) {
				var testData = WebSocketManager.createSendObj( Class, SubClassName, Command, Data, resolve, reject);	
				WebSocketManager.sendAction( testData );
			});
		},
		...
		createSendObj:function( ClassName, SubClassName, CommandName, CommandData, SuccessCB, ErrorCB ) {
			...
			if ( typeof ( ClassName ) !== 'string' ) { throw("WebSocketManager: createSendObj: Error: ClassName is not a string"); }
			if ( typeof ( SubClassName ) !== 'string' ) { throw("WebSocketManager: createSendObj: Error: SubClassName is not a string"); }
			if ( typeof ( CommandName ) !== 'string' ) { throw("WebSocketManager: createSendObj: Error: CommandName is not a string"); }
			if ( typeof ( CommandData ) !== 'object' ) { throw("WebSocketManager: createSendObj: Error: CommandData is not an Object"); }
			if ( typeof ( SuccessCB ) !== 'function' ) { throw("WebSocketManager: createSendObj: Error: SuccessCB is not a function"); }
			if ( typeof ( ErrorCB ) !== 'function' ) { throw("WebSocketManager: createSendObj: Error: ErrorCB is not a function"); }

			var evt_no = WebSocketManager.evt_cnt++;
        	
        	var cbStack = WebSocketManager.getStackTrace( (new Error()) );
			...
			var sendObj = {
				...
				_successCB: function( Data, MsgDate ) {	
					...
					var printMsg = "WebSocketManager: Callback: " + 
					...
					console.info("WSKT", printMsg, WebSocketManager.responseToString( this ) );
				...
				_errorCB: function( Data, MsgDate ) {
				 ...
				 // 	var responseMsg = "\nResponse: " + JSON.stringify( Data, null, '\t');
					...
					console.warn("WSKT", printMsg, WebSocketManager.responseToString( this ) );
		...
		_createRecvObj:function( JsonData ) {
			...
			WebSocketManager.recvQueue[JsonData.tid] = recvObj;
			WebSocketManager.logDb.push(recvObj);		// Put in the logging Array (DB like) 
		...
		//   events arriving at WebSocketManager from NetComposer.  There can be hundreds
		...
		//--------------------------------------------------------
		//  handleEvent - Parses an object from WebSocketManager and sends it to 
		//     registered handlers.
		//--------------------------------------------------------
		handleEvent:function( WsEventObj ) {
			...
			if ( ( WsEventObj.wsRecvData.class === "core" ) && ( WsEventObj.wsRecvData.cmd === "event") && ( !!WsEventObj.wsRecvData.data ) ) {		// This seems to be an Event from NetComposer
				...
				if ( !!WsEventObj.wsRecvData.data.class ) { 
					...
					throw( "WebSocketManager: handleEvent: Error: No Data.class field in this event: ", WsEventObj.wsRecvData );
				}
				...
				if ( !! this.classManagers[ comboName1 ] ) {
					console.localDebugOnly("WSKT", "WebSocketManager: handleEvent: handleEvent: Using comboName1 - " + comboName1 );
					this.classManagers[ comboName1 ]( WsEventObj.wsRecvData );
					return true;
				} else if ( !! this.classManagers[ comboName2 ] ) {
					console.localDebugOnly("WSKT", "WebSocketManager: handleEvent: handleEvent: Using comboName2 - " + comboName2 );
					this.classManagers[ comboName2 ]( WsEventObj.wsRecvData );
					return true;
				} else if ( !! this.classManagers[ comboName3 ] ) {
					console.localDebugOnly("WSKT", "WebSocketManager: handleEvent: handleEvent: Using comboName3 - " + comboName3 );
					this.classManagers[ comboName3 ]( WsEventObj.wsRecvData );
					return true;
				} else {
					// console.localErrorOnly("WSKT", "WebSocketManager: handleEvent: Error: No Handler for this event: " + JSON.stringify( WsEventObj.wsRecvData, null, '\t' ) );
					EventBus.dispatch( NcEvents.onRemoteLoggerError, ["WSKT", 
						"WebSocketManager: handleEvent: Error: No Handler for this event.\n" +
					...
					this.respondError( WsEventObj, 333, "WebSocketManager: handleEvent: No Handler available for this Event" + JSON.stringify( WsEventObj, null, '\t') );

				}

			} else {		// This does NOT seem to be an Event from NetComposer
				throw( "WebSocketManager: handleEvent: Error: This event is not valid: ", WsEventObj.wsRecvData );
			}
		},
	...
	var WebSocketManager = new WebSocketManagerClass();
